...
import { Badge, Collapse, CollapseProps, Flex, Result, Space, Steps, Tooltip } from 'antd';
...
import { DocumentStatusTag } from './document-forms/DocumentStatusTag';
  ...
  const items: CollapseProps['items'] = [
    {
      key: 'passport',
      label: 'Паспорт',
      extra: <DocumentStatusTag status={data?.passport?.status} message={data?.passport?.message} />,
      children: <PassportForm id={user?.id || ''} data={data?.passport} setUpdateNeeded={setUpdateNeeded} />
    },
    {
      key: 'snils',
      label: 'СНИЛС',
      extra: <DocumentStatusTag status={data?.snils?.status} message={data?.snils?.message} />,
      children: <SnilsForm id={user?.id || ''} data={data?.snils} setUpdateNeeded={setUpdateNeeded} />
    },
    {
      key: 'education',
      label: 'Диплом',
      extra: <DocumentStatusTag status={data?.education?.status} message={data?.education?.message} />,
      children: <p></p>
    },
    {
      ...
      extra: (
        <DocumentStatusTag
          status={data?.document_on_change_of_surname?.status}
          message={data?.document_on_change_of_surname?.message}
        />
      ),
      children: <p></p>
    },
    ...
    },
    {
      ...
      extra: <DocumentStatusTag status={data?.opd?.status} message={data?.opd?.message} />,
      children: <p></p>
    }
  ];
  const getData = () => {
